...
void drawLineOverlap(unsigned int aXStart, unsigned int aYStart, unsigned int aXEnd, unsigned int aYEnd, uint8_t aOverlap, uint16_t aColor) {
  ...
  */
  ...
  if ((aXStart == aXEnd) || (aYStart == aYEnd)) {
    ...
    tft.drawLine(aXStart, aYStart, aXEnd, aYEnd, aColor);
    //tft.fillRect(min(aXStart, aXEnd), min(aYStart, aYEnd), abs(aXEnd - aXStart), abs(aYEnd - aYStart), aColor);
  } else {
    ...
    tft.drawPixel(aXStart, aYStart, aColor);
    if (tDeltaX > tDeltaY) {
      ...
      while (aXStart != aXEnd) {
        ...
        if (tError >= 0) {
          ...
          if (aOverlap & LINE_OVERLAP_MINOR) {
            ...
            tft.drawPixel(aXStart - tStepX, aYStart, aColor);
          }
      ...
      while (aYStart != aYEnd) {
        ...
        if (tError >= 0) {
          ...
          if (aOverlap & LINE_OVERLAP_MINOR) {
            ...
            tft.drawPixel(aXStart, aYStart - tStepY, aColor);
          }
          tError -= tDeltaYTimes2;
        }
        tError += tDeltaXTimes2;
...
*/
/*
  void drawThickLineSimple(unsigned int aXStart, unsigned int aYStart, unsigned int aXEnd, unsigned int aYEnd,unsigned int aThickness, uint8_t aThicknessMode, uint16_t aColor) {
  int16_t i, tDeltaX, tDeltaY, tDeltaXTimes2, tDeltaYTimes2, tError, tStepX, tStepY;
